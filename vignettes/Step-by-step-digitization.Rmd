...
fun_ggplot(step1[,,3]) +
  geom_vline(aes(xintercept = c(min(step2$xaxis))), color = "green") +
  geom_vline(aes(xintercept = c(max(step2$xaxis))), color = "green") +
  geom_hline(aes(yintercept = c(max(step2$yaxis))), color = "green") +
  geom_hline(aes(yintercept = c(min(step2$yaxis))), color = "green") +
  theme_bw()
...
step4 <-fun_colordetect(fig.df = step3, num_curves = 3, black_marks = F)
...
step5[[2]] %>%
 ggplot(aes(x = x, y =y, color =as.factor(potential) )) +
 geom_point()
