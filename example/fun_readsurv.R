...
# fun_readsurv(FilePath)
  # Inputs:
    # FilePath: location of jpeg survival curve
  # Returns:
      # List w/ 3 objects
        #fig.rgb = RGB array
        #fig.BW  = Black and white Matrix
        #fig.hsv = an Array w/ H S V formats
  # Dependency:
      # JPEG
      # png
      # imager 
...
if(str_detect(FilePath , "jpeg|jpg")){
fig.arr <- readJPEG(FilePath,native = F)
} else if(str_detect(FilePath , "png")){
fig.arr <-  readPNG(FilePath,native = F)
} else(
  stop("Couldn't read in image are you sure the file is saved as a jpeg, jpg, or png?")
)
  
## Turning BW into an RGB image
if(length(dim(fig.arr)) == 1 ){
